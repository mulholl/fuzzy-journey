...
int main(int argc, char *argv[]){
	...
	bool verbose; // If this is true the indices for each of the chosen words are given
	...
	opts.addOpt('w', "words", "Number of words in passphrase", inpopts::value<int>(num_words).numArgs(1).defaultValue(6).setMin(1));
	opts.addOpt('v', "verbose", "Verbose mode - lists the indices of the words chosen from the word list", inpopts::value<bool>(verbose).defaultValue(false));
	...
	if (opt == EFF_LARGE){
		...
		passphrase = dicewareGeneratePassphrase(wl, num_words, passphrase_with_info, RNG);
